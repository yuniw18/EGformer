    ...
    def __init__(self, root,
        ...
        for dir_sub in dir_path:
            ...
            for path in sub_path_list:
                dir_sub_dir.append(os.path.join(sub_path,path,'panorama'))
                

        for final_path in dir_sub_dir:
            self.image_paths.append(os.path.join(final_path,'full/rgb_rawlight.png'))                    
            self.cor_paths.append(os.path.join(final_path,'layout.txt'))                    
...
#                                self.img_fnames[idx])
        ...
        H, W = img.shape[:2]
        with open(self.cor_paths[idx]) as f:
        ...
        dist_o = cdist(corx.reshape(-1, 1),
                       np.arange(img.shape[1]).reshape(-1, 1),'minkowski',p=1)
        dist_r = cdist(corx.reshape(-1, 1),
                       np.arange(img.shape[1]).reshape(-1, 1) + img.shape[1],'minkowski',
                       p=1)
