...
def main_worker(gpu, ngpus_per_node, config):
    ...
    if config.train_set == 'S3D':
        Data_sampler = S3D_loader(config.S3D_path,transform = transform_s3d,transform_t = transform_s3d)

    elif config.train_set == 'Pano3D':
        Data_sampler = Pano3D(
    ...
    elif config.train_set == 'Concat':
        Data_sampler = torch.utils.data.ConcatDataset([
             S3D_loader(config.S3D_path,transform = transform_s3d,transform_t = transform_s3d),
             Pano3D(
                root = config.pano3d_root,
                part=config.pano3d_part,
                split=config.pano3d_split,
